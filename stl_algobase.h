...
__copy_trivial(const _Tp* __first, const _Tp* __last, _Tp* __result) {
  memmove(__result, __first, sizeof(_Tp) * (__last - __first));	//memmove用于从first拷贝last-first个字符到result，如果目标区域和源区域有重叠的话，memmove能够保证源串在被覆盖之前将重叠区域的字节拷贝到目标区域中。但复制后src内容会被更改。但是当目标区域与源区域没有重叠则和memcpy函数功能相同。
...
inline _BidirectionalIter2 __copy_backward(_BidirectionalIter1 __first, 
                                           ...
                                           _Distance*)
{
  while (__first != __last)	//从最后一个倒着往前拷贝。
    *--__result = *--__last;
  return __result;
}
...
pair<_InputIter, _OutputIter> __copy_n(_InputIter __first, _Size __count,
  ...
  return pair<_InputIter, _OutputIter>(__first, __result);	// 返回copy_n结束后first的位置和result的位置。
}
...
pair<_InputIter1, _InputIter2> mismatch(_InputIter1 __first1,
  ...
  while (__first1 != __last1 && *__first1 == *__first2) {
    ++__first1;
    ++__first2;
  }	// 查找first1和first2两个区间前面相等的部分，返回第一个不相等的地方。
  return pair<_InputIter1, _InputIter2>(__first1, __first2);
}
//__binary_pred 二元判断函数。
...
inline bool equal(_InputIter1 __first1, _InputIter1 __last1,
  ...
  for ( ; __first1 != __last1; ++__first1, ++__first2)
    if (*__first1 != *__first2)	// 有一个不等的地方，返回false
      return false;
  return true;
}
//__binary_pred
...
bool lexicographical_compare(_InputIter1 __first1, _InputIter1 __last1,
  ...
  for ( ; __first1 != __last1 && __first2 != __last2
        ; ++__first1, ++__first2) {
    if (*__first1 < *__first2)	//区间1 的字典序小 ，返回true
      return true;
    if (*__first2 < *__first1)	//2的字典序小，返回false
      return false;
  }
  return __first1 == __last1 && __first2 != __last2;	//全部比较完，哪个长，哪个字典序大，一样长的话返回2大，也就是false。// first1==last1&&first2==last2
}
                        ...
                        const unsigned char* __last2)
{
  ...
  const int __result = memcmp(__first1, __first2, min(__len1, __len2));	//对于char类型，由于是单字节的，直接使用memcmp即可。
  return __result != 0 ? __result < 0 : __len1 < __len2;
  // result==0表示min部分二者相同，len1小于len2的话，说明其字典序小，返回true。。否则，result小于零表示len1字典序小，返回true
}
                                   ...
                                   _InputIter2 __first2, _InputIter2 __last2)
{
  ...
  if (__first2 == __last2) {	// 如果len2<=len1,
    return !(__first1 == __last1);	// len1<=len2,二者完全相等，返回0. // 否则返回1表示第二个字典序小。
  }
  else {	// 否则第一个字典序小。
    return -1;	
  }
                               ...
                               const unsigned char* __last2)
{
  ...
  return __result != 0 ? __result 	// result不等于零，直接返回result。
                       : (__len1 == __len2 ? 0 : (__len1 < __len2 ? -1 : 1));	// 否则根据长度判断。
}
