...
public:
  ...
  static bool eq(const _CharT& __c1, const _CharT& __c2) //euqal?
    { return __c1 == __c2; }
  static bool lt(const _CharT& __c1, const _CharT& __c2) 	//less than?
    { return __c1 < __c2; }

  static int compare(const _CharT* __s1, const _CharT* __s2, size_t __n) {  // cmopare 相等返回零，s1<s2返回-1，否則返回1
  ...
  static size_t length(const _CharT* __s) {		// 計算長度
  ...
  static const _CharT* find(const _CharT* __s, size_t __n, const _CharT& __c)		//查找s中第一次出現c的位置
  {
    for ( ; __n > 0 ; ++__s, --__n)
      if (eq(*__s, __c))
        return __s;
  ...
  static _CharT* move(_CharT* __s1, const _CharT* __s2, size_t __n) {
    memmove(__s1, __s2, __n * sizeof(_CharT));		//memmove() 与 memcpy() 类似都是用来复制 src 所指的内存内容前 num 个字节到 dest 所指的地址上。不同的是，memmove() 更为灵活，当src 和 dest 所指的内存区域重叠时，memmove() 仍然可以正确的处理，不过执行效率上会比使用 memcpy() 略慢些。
    return __s1;
  }
  ...
  static _CharT* assign(_CharT* __s, size_t __n, _CharT __c) {		//全部設置為 __c
    for (size_t __i = 0; __i < __n; ++__i)
      __s[__i] = __c;
    return __s;
  }

  static int_type not_eof(const int_type& __c) {		//是否到達文件尾
